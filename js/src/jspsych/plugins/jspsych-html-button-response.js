...
jsPsych.plugins["html-button-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    //show prompt if there is one -- Sean Devine (11-22-2020): moved prompt to top
    ...
    // added by Sean
    if(choices.length < 3) ncol = choices.length ;
    if(choices.length >= 3) ncol = 3;
    html += '<div class="grid-container" style="grid-template-columns: repeat('+ncol+', minmax(0, 1fr));">';
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      ...
      html+= '<div class="grid-item" id="jspsych-html-button-response-button-' + i +'" data-choice="'+i+'">'+str+'</div>'
